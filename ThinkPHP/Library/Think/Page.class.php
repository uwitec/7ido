...
//     );
    ...
    // 分页显示定制
    private $config  = array(
        'header' => '<span class="rows">Page %NOW_PAGE% of %TOTAL_PAGE% </span>',
        'prev'   => '<<',
        'next'   => '>>',
        'first'  => '1...',
        'last'   => '...%TOTAL_PAGE%',
        'theme'  => '%HEADER%  %UP_PAGE% %LINK_PAGE% %DOWN_PAGE% %END%',
    );
    ...
    public function dispaly() {
     ...
     $the_end = '';
     if($this->totalPages > $this->rollPage && ($this->nowPage + $now_cool_page) < $this->totalPages){
      $the_end = '<a class="nextpostslink" href="' . $this->url($this->totalPages) . '">' . $this->config['last'] . '</a>';
     }
